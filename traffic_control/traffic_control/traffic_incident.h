...
static const map<int, double> car_Type = { {0,1.0}, {1,1.0},{2,1.5},{3,2.5} };          //不同类型车辆的换算系数
static const double time_Reaction = 1.5, dist_Safe = 1.5, surface_Ratio = 0.8;                                    //刹车的反应时间，两车安全距离，沥青路面附着系数
static const map<int, double> speed_Deceleration_Value = { {1,6.05},{2,5.6},{3,4.75} };  //不同类型车辆的刹车加速度值                                               
...
static const int    MaxLimitedVehicleNum = 5;					//默认10
...
public:
	Vehicle() {}
	Vehicle(double msec,double mnsec,int mid,int mlane_Num, uint32_t mlabel, int mtype, double mx, double my, double mvx, double mvy, double mlength, double mwidth ) :
...
public:
	Vehicle_Extent() {};
	Vehicle_Extent(double msec,double mnsec,int mid,int mlane_Num, uint32_t mlabel, int mtype, double mx, double my, double mvx, double mvy, double mlength, double mwidth) :
...
public:
	...
	Vehicleincident_Detection(double msec, double mnsec, int mid, int mlane_Num, uint32_t mlabel, int mtype, double mx, double my, double mvx, double mvy, double mlength, double mwidth ) :
...
public:
	...
	void caculation_Index();
	double time_Headway_Calibration(int cartype);        //标定车头时距值
public:
...
public:
	...
	Max_Queue_Caculation(int mlanes_Num,bool msection_Flag, double mspeed_Start, double mspeed_end, int mmin_Size, map<int, bool> mqueue_continue, map<string, vector<vector<Point>>> mdetect_Config) :Headway_Density_Caculation(mlanes_Num,msection_Flag, mdetect_Config),
		...
		max_Headway_Queue = speed_Queue_Start * time_Reaction + pow(speed_Queue_Start, 2.0) / (2 * speed_Deceleration_Value.at(1)* surface_Ratio) + 5 + dist_Safe;  //vt+pow(v,2)/(2a)+length+dist_safe
		max_Headway_Slow = speed_Queue_End * time_Reaction + pow(speed_Queue_End, 2.0) / (2 * speed_Deceleration_Value.at(1)* surface_Ratio) + 5 + dist_Safe;
