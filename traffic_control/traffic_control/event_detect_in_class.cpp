...
//detector_target_info_t Event_detect::recieve_data()
//{
//
//    return ;
//}
...
void Event_detect::handle_data(detector_target_info_t detector_target_info){
    ...
    for (int i = 0; i < 1; i++)
    {
        ...
        uint32_t label = 5;
        float object_px = detector_target_info.radar_px;
        float object_py = detector_target_info.radar_py;
        float object_vx = detector_target_info.radar_vx;
        float object_vy = detector_target_info.radar_vy;
        float length = detector_target_info.dimensions_x;
        float width = detector_target_info.dimensions_y;
        float lat = detector_target_info.targetLatitude;
        float lon = detector_target_info.targetLongitude;
        int source = 1;

        // std::cout<<"plate_num:"<<plate_num<<std::endl;
        ...
        if (veh_test.speed > MaxSpeedUpper || veh_test.id <= 0)
            continue;
        ...
        intersection_Devices_Detect[source].speed_test.current_Time = veh_test.timestamp;
        //intersection_Devices_Detect[source].stops_test.current_Time = veh_test.timestamp;
        ...
        if (veh_test.lane_Num <= (10 + intersection_Devices_Detect[source].lanes_Num)) {
            ...
            if (source == 0)
                //printf("sec: %f, nsec: %f, ID: %d, lane_No: %d, pos_x: %f, pos_y: %f, Vx: %f,Vy: %f ,speed: %f,lat: %f,lon: %f,sensor_id: %d,plate_num: %s \n",sec,nsec,id,lane_id,object_px,object_py,object_vx,object_vy,veh_test.speed*3.6,lat,lon,source,plate_num.c_str());
                intersection_Devices_Detect[source].volume_test.get_Vehicles_Info(veh_test);                      //采集车辆，用于计算流量
            intersection_Devices_Detect[source].speed_test.get_Vehicles_Info(veh_test);                       //采集车辆，用于计算空间平均速度
            intersection_Devices_Detect[source].capacity_test.get_Vehicles_Info(veh_test);                    //采集车辆，用于计算平均车头时距和车道通行能力
            intersection_Devices_Detect[source].max_queue_test.get_Vehicles_Info(veh_test);                   //采集车辆，用于计算速度、密度、车头间距和排队长度
        }
        intersection_Devices_Detect[source].vehs_test.push_back(veh_test);
    }
    //printf("\n");
    for (auto it = intersection_Devices_Detect.begin(); it != intersection_Devices_Detect.end(); it++) {
        ...
        //printf("time_stamp : %f, source : %d, zone_volume : %d , speed : %f\n",time_stamp_nsec,it->first,it->second.max_queue_test.zone_volume,it->second.max_queue_test.ave_Time_Speed);

        it->second.gatewaywarninglable = Gateway(it->second.vehs_test, it->second.lanes_Num);                  //是否存在汇入预警
        it->second.congestionlabel = Congestion(it->second.vehs_test, it->second.max_queue_test, it->second.max_queue_test.zone_volume);             //是否发生拥堵
    }
