...
double Capacity_Caculation::time_Headway_Calibration(int cartype) {     //根据车型和限速，标定车头时距值
...
void Capacity_Caculation::caculation_Index(){
	...
	for (auto it = sat_Time_Headway.begin(); it != sat_Time_Headway.end(); it++) {
		if ((*it).second > 0.0)
			...
			capacity_temp = 3600 / time_Headway_Calibration(1);         //采用小轿车作为车头时距计算的标准
...
void Vehicleincident_Detection::Retrograde()
{
	...
	// else if(lane_Num>30&&lane_Num<=40)
	// {
	// 	if(radar_vy < 0)   
	// 	{
	// 		retrograde = true;
...
bool Gateway(vector<Vehicleincident_Detection> vehs_test, int lanes_Num)
{
	...
	if (vehs_test.size() >= 1)
	{
		...
		{	
			...
			else if (vehs_test[j].lane_Num == 10+lanes_Num) //==&&车道编号需提前定义
			{
...
int Congestion(vector<Vehicleincident_Detection> vehs_test, Max_Queue_Caculation max_queue_test,int zone_volume) //需要修改
{
	int congestionlevel = 0;
	if (zone_volume >= MaxLimitedVehicleNum)
	{
